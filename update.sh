...
resolve_update_archive_path() {
    local archive_name="x-ui-linux-$(arch).tar.gz"

    # FIX: if local release archive exists in current dir, use it directly to keep update workflow simple.
    if [[ -f "${cur_dir}/${archive_name}" ]]; then
        echo "${cur_dir}/${archive_name}"
        return 0
    fi
...
update_x-ui() {
    ...
    local_archive_path=$(resolve_update_archive_path)
    if [[ -n "${local_archive_path}" ]]; then
        echo -e "${green}Local package detected: ${local_archive_path}${plain}"
        cp -f "${local_archive_path}" "${xui_folder}-linux-$(arch).tar.gz"
        tag_version="local-build"
    else
        tag_version=$(get_latest_tag_version)
        if [[ ! -n "$tag_version" ]]; then
            _fail "ERROR: Failed to fetch x-ui version from ${github_repo}. Simple mode: place x-ui-linux-$(arch).tar.gz near update.sh."
        fi
        ...
        if [[ $? -ne 0 ]]; then
            ...
            ${curl_bin} -4fLRo ${xui_folder}-linux-$(arch).tar.gz ${github_release_base}/${tag_version}/x-ui-linux-$(arch).tar.gz 2>/dev/null
            if [[ $? -ne 0 ]]; then
