...
resolve_install_archive_path() {
    local archive_name="x-ui-linux-$(arch).tar.gz"

    # FIX: if local release archive exists in current dir, use it directly to keep installation simple for fork users.
...
install_x-ui() {
    ...
    local_archive_path=$(resolve_install_archive_path)
    if [[ -n "${local_archive_path}" ]]; then
        ...
        if [[ ! -n "$tag_version" ]]; then
            ...
            echo -e "${yellow}Simple mode: put x-ui-linux-$(arch).tar.gz near install.sh and run again.${plain}"
            echo -e "${yellow}Advanced mode: set XUI_TAG_VERSION manually, e.g. export XUI_TAG_VERSION=v2.5.0${plain}"
            exit 1
        fi
